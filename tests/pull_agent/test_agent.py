...
async def test_handle_message_ok():
    ...
    with patch("big_data_model.pull_agent.agent._llm_chat", new=AsyncMock(return_value=fake)):
...
async def test_handle_message_chat_api_raises():
    ...
    with patch("big_data_model.pull_agent.agent._llm_chat", new=AsyncMock(return_value=fake)), \
         patch("big_data_model.pull_agent.agent.add_sys_manager_to_chat", side_effect=boom):
...
async def test_handle_message_parse_error():
    with patch("big_data_model.pull_agent.agent._llm_chat", new=AsyncMock(return_value="not json")):
